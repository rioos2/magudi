...
echo gitlab.com/rioos/magudi/pkg/apimachinery/registered > ${dir}/packages.txt
echo gitlab.com/rioos/magudi/pkg/runtime/serializer >> ${dir}/packages.txt
echo gitlab.com/rioos/magudi/pkg/runtime/serializer/yaml >> ${dir}/packages.txt
echo gitlab.com/rioos/magudi/pkg/runtime/serializer/streaming >> ${dir}/packages.txt
echo gitlab.com/rioos/magudi/pkg/runtime/serializer/recognizer/testing >> ${dir}/packages.txt
go list -f {{.Deps}} gitlab.com/rioos/magudi/pkg/apimachinery/registered | sed -e 's/ /\n/g' - | grep k8s.io | grep -v vendor >> ${dir}/packages.txt
go list -f {{.Deps}} gitlab.com/rioos/magudi/pkg/runtime/serializer | sed -e 's/ /\n/g' - | grep k8s.io | grep -v vendor >> ${dir}/packages.txt
go list -f {{.Deps}} gitlab.com/rioos/magudi/pkg/runtime/serializer/yaml | sed -e 's/ /\n/g' - | grep k8s.io | grep -v vendor >> ${dir}/packages.txt
go list -f {{.Deps}} gitlab.com/rioos/magudi/pkg/runtime/serializer/streaming | sed -e 's/ /\n/g' - | grep k8s.io | grep -v vendor >> ${dir}/packages.txt
# used by tests
echo gitlab.com/rioos/magudi/pkg/util/diff >> ${dir}/packages.txt
go list -f {{.Deps}} gitlab.com/rioos/magudi/pkg/util/diff | sed -e 's/ /\n/g' - | grep k8s.io | grep -v vendor >> ${dir}/packages.txt
...
while read package; do
	unprefix_package=$(echo ${package} | sed 's|gitlab.com/rioos/magudi/||g')
...
find ${KUBE_ROOT}/staging/src/k8s.io/apimachinery -name "*.go" | xargs sed -i 's|gitlab.com/rioos/magudi|k8s.io/apimachinery|g'
...
while read package; do
	...
	new_package=$(echo ${package} | sed 's|gitlab.com/rioos/magudi|k8s.io/apimachinery|g')
